...
function WHome(props) {
  const api = "e2dcbc32141e7bc1be24918abe848bb1";
  const unsplash_api = "MDyStk7xtRGFe_VclM1_AMOfiigsD64XZ6M5UIE55HA";
  useEffect(() => {
  ...
  const [mon, setmon] = useState(["18" , "10d"]);
    ...
    useEffect(async () => {
      ...
      const resp3 = await fetch(`https://api.unsplash.com/search/photos?query=${props.entry}&client_id=${unsplash_api}`);
      ...
      if(images.results.length !== 0){
        ...
        const pix = [{img : `${images.results[0].urls.small}`} ,
      {img : `${images.results[1].urls.small}`}  ,
      {img : `${images.results[2].urls.small}`}  ,
      {img : `${images.results[3].urls.small}`}                
    ]
      props.setimg(pix);
      } 
      if(images.results.length === 0){
